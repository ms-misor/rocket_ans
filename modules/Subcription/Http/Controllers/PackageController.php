...
class PackageController extends Controller
{
    ...
    public function index()
    {
        return view('subcription::packages.index',[
            'mpackages'          => Package::with('modules')->where('package_type',1)->orderBy('id','desc')->get(),
            'modules'            => Module::where('type',1)->orderBy('id','desc')->get(),
        ]);
    }
    ...
    public function create()
    {
        return view('subcription::packages.create',[
            'modules'           => Module::orderBy('id','desc')->where('type',1)->get(),
    ...
    public function store(PackageRequest $request)
    {
        $request->validate([
            ...
            'module_id'         => 'required|array|min:2'

        ],[ 
            'required' => ':attribute field is required',
            'min'      => 'Atleast two module should have'
        ]);
    ...
    public function getPackageInfoByType(Request $request)
    {

        $package = Package::where('package_type',$request->package_type)->orderBy('id','desc')->get();
       
        $packages= '<option value="">Select package</option>';
        foreach($package as $item){
            $packages .='<option value="'.$item->id.'">'.$item->title.'</option>';
        }
